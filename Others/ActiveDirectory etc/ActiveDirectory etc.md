...
Update-MgUser `
    ...
    -PasswordProfile $PasswordProfile
```
セルフサービスパスワードリセット（SSPR）も仕組みとしてある
　⇒本人確認の後、自分で再設定
...
故障等の障害で、認証不可になるのを防ぐ為、
2台以上の複数DCで運用する事が多い。
複数DCの場合、データ同期処理として、
レプリケーションされる（＝冗長化）

例として「company.local」というADドメインを作った場合、
内部的には以下の形式で管理される
```text
DC=company
DC=local
...
```
配下にOUを作成した場合、LDAP形式では「OU=○○,DC=company,DC=local」になる。
...
## Windows Server管理
AD参加前のWindowsは、PC自身がアカウントを管理（＝ローカルアカウント）
...
netstat -ano
```
-a：すべての接続状態（待ち受け中も含む）を表示する
-n：名前（ドメイン名など）ではなく、数字（IPアドレスやポート番号）で表示する（処理が早くなる）
-o：その通信を行っているプログラムの識別番号（PID：プロセスID）を表示する

不審な通信（ウイルスなど）が勝手に行われていないか確認する
「ポートが重複してアプリが起動できない」というときに、
どのプログラム（PID）がそのポートを占有しているかを特定する。
...
多くの機器が対応
```
一般家庭は、WPA2-PSK（共通のパスワード）が多いが、
企業はWPA2-Enterpriseが多い
　⇒個人ごと（または端末ごと）に、IDとパスワード、
　　あるいは電子証明書を使って個別に認証する方式
　　認証サーバー(RADIUS)を用い、802.1X認証を行う
　　　※PC ⇒ AP ⇒ RADIUS ⇒ AD
　　パスワード漏洩がなく、アカウント（ID）を
　　無効化するだけで接続不可にできる
